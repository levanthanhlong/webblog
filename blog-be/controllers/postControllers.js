import { uploadAudio, uploadPicture } from "../middleware/uploadPictureMiddleware";
...
const updatePost = async (req, res, next) => {
  try {
    ...
    const upload = uploadPicture.fields([
    ...
    upload(req, res, async function (err) {
      if (err) {
        ...
        if (req.files["audioFile"]) {
          let audioFilename = post.audio;
          if (audioFilename) {
            fileRemover(audioFilename);
          }
          post.audio = req.files["audioFile"][0].filename;
        }
  ...
  try {
    ...
    const [results, totalCount] = await Promise.all([
      Post.find(where)
        ...
        .sort({ updatedAt: "desc" }),
      Post.find(where).countDocuments(),
    ]);
    ...
    res.header({
      "x-filter": filter,
      "x-totalcount": JSON.stringify(totalCount),
      "x-currentpage": JSON.stringify(page),
      "x-pagesize": JSON.stringify(pageSize),
      "x-totalpagecount": JSON.stringify(totalPages),
    });
